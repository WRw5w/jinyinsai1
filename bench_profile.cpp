...
int main(int argc,char**argv){
 ...
 double __tsort=0,__tset=0;long long nOpt=0,nExp=0,nSortItems=0,nSorts=0,nLevels=0,nTrials=0,nStatesKept=0;double tSort=0;mt19937 rng(stoul(argv[4]));int beam=stoi(argv[5]);long attempts=0,accepted=0;
 ...
 while(chrono::duration<double>(chrono::steady_clock::now()-started).count()<seconds){
  ...
  for(int trial=0;trial<8;trial++){
   ...
   for(int t=0;t<(int)ids.size()&&!states.empty();t++){nLevels++;
    ...
    for(auto&s:states)for(auto&op:options){
     ...
     nExp++;next.push_back(n);
    }
    if(next.size()>(size_t)beam){
     nSorts++;nSortItems+=next.size();auto __t0=chrono::steady_clock::now();
     auto __q0=chrono::steady_clock::now();
     sort(next.begin(),next.end(),[](const State&a,const State&b){return a.rank<b.rank;});
     __tsort+=chrono::duration<double>(chrono::steady_clock::now()-__q0).count();
     ...
     __tset+=chrono::duration<double>(chrono::steady_clock::now()-__q1).count();
     next.swap(kept);double __d=chrono::duration<double>(chrono::steady_clock::now()-__t0).count();tSort+=__d;
     static double __ts=0,__td=0;{static chrono::steady_clock::time_point __m0,__m1;if(nSorts%1000000==0)__m0=chrono::steady_clock::now();}

    }
    nStatesKept+=next.size();
    states.swap(next);
   }
   for(auto&s:states){
 ...
 cerr<<"PROFILE attempts="<<attempts<<" trials="<<nTrials<<" levels="<<nLevels<<" options="<<nOpt<<" expansions="<<nExp<<"\n"     <<"        sortOnlySec="<<__tsort<<" setDedupSec="<<__tset<<" copyBackSec="<<(tSort-__tsort-__tset)<<"\n"     <<"        sorts="<<nSorts<<" sortedItems="<<nSortItems<<" sortSeconds="<<tSort<<" keptSum="<<nStatesKept<<"\n"     <<"        perAttempt trials="<<(double)nTrials/attempts<<" levels="<<(double)nLevels/attempts<<" optionsPerLevel="<<(nLevels?(double)nOpt/nLevels:0.)<<"\n"     <<"        perSort avgItems="<<(nSorts?(double)nSortItems/nSorts:0.)<<" sortPctOfWall="<<(tSort/seconds*100)<<"%"<<"\n";
