// Beam search for two rounds with different parallel counts. No external solver.
#include <algorithm>
#include <array>
#include <chrono>
#include <cmath>
...
#include <vector>
using namespace std;
struct Order{int q,cap,pm,delivered=0;double size,mu;};
...
Tot value(const Row&r,int bid){Tot t;double L=0;for(auto [i,k]:r.cuts){t.k+=k;L+=k*orders[i].size;}t.k++;t.f=L*r.p*orders[r.cuts[0].first].mu;t.r=r.nb*blanks[bid];return t;}
...
int main(int argc,char**argv){
 ...
 for(auto&o:orders)in>>o.q>>o.cap>>o.size>>o.mu>>o.pm;
  ...
  for(int trial=0;trial<8;trial++){
   ...
   for(int t=0;t<(int)ids.size()&&!states.empty();t++){
    ...
    for(int a=0;a<=amax;a++){
     ...
     for(int b=blo;b<=bhi;b++)options.push_back({a,b,a*o.size,b*o.size,(a*p1+b*p2)*o.size*mu});
    }
    ...
    for(auto&s:states)for(auto&op:options){
     ...
     State n=s;n.l1+=op.l1;n.l2+=op.l2;n.f+=op.f;n.k+=op.a+op.b;n.a[t]=op.a;n.b[t]=op.b;
