// Same search as bench_base_det.cpp, but the beam-pruning step no longer sorts
// every expansion and no longer de-duplicates through std::set.
//
// Motivation (measured on runs/kh5a/engine_input.txt, 15 s, beam 48):
//   sortSeconds = 8.81 / 15.0 wall (58.8 %), setDedupSec = 2.76 (18 %)
//   sorts = 269568, sortedItems = 92113150, expansions = 94821844
// i.e. ~77 % of the wall time is spent ordering ~342-element arrays and
// inserting their bucket keys into a red-black tree.  The arithmetic itself is
// nearly free, so a GPU would have nothing to accelerate; the win is to stop
// doing O(n log n) + O(n log n) work where O(n) suffices.
//
// Replacement: bucket keys (int(l1*2), int(l2*2)) are bounded by 2*148 = 296,
// so a direct-indexed table (512 x 512) plus a generation stamp records, in one
// pass, the minimum-rank state of every bucket.  Selection of the best `beam`
// buckets is then nth_element over the (>= beam) touched buckets, i.e. O(n).
// Identical keep-set semantics: the current code also keeps the first
// (== minimum-rank) state of each bucket.
#include <algorithm>
#include <array>
#include <chrono>
#include <cmath>
...
const double KNIFE_BUDGET=89990.;
...
// ---- O(n) beam filter -------------------------------------------------------
const int KEYW=512;                       // keys are int(l*2) <= 296
static vector<int>   bkStamp(KEYW*KEYW,-1);
static vector<double>bkRank (KEYW*KEYW,0.);
static vector<int>   bkPos  (KEYW*KEYW,0);
static vector<int>   touched;             // only entries written by this generation
static int gen=0;
static long long nFallback=0;             // keys outside the table (should stay 0)

static void keep_top_beam(vector<State>&next,int beam){
 touched.clear();++gen;
 for(int idx=0;idx<(int)next.size();idx++){
  const State&s=next[idx];
  int i1=int(s.l1*2),i2=int(s.l2*2);
  if(i1<0||i1>=KEYW||i2<0||i2>=KEYW){nFallback++;continue;}
  int key=i1*KEYW+i2;
  if(bkStamp[key]!=gen){bkStamp[key]=gen;bkRank[key]=s.rank;bkPos[key]=idx;touched.push_back(key);}
  else if(s.rank<bkRank[key]){bkRank[key]=s.rank;bkPos[key]=idx;}
 }
 int keep=min((int)touched.size(),beam);
 if(keep<(int)touched.size()){
  nth_element(touched.begin(),touched.begin()+keep,touched.end(),
              [](int x,int y){return bkRank[x]<bkRank[y];});
  touched.resize(keep);
 }
 sort(touched.begin(),touched.end(),[](int x,int y){return bkRank[x]<bkRank[y];});
 vector<State>kept;kept.reserve(keep);
 for(int key:touched)kept.push_back(next[bkPos[key]]);
 next.swap(kept);
}
...
int main(int argc,char**argv){
 ...
 while(chrono::duration<double>(chrono::steady_clock::now()-started).count()<seconds
  ...
  for(int trial=0;trial<8;trial++){
   ...
   for(int t=0;t<(int)ids.size()&&!states.empty();t++){
    ...
    if(next.size()>(size_t)beam)keep_top_beam(next,beam);
    states.swap(next);
   }
   for(auto&s:states){
 ...
 double wall=chrono::duration<double>(chrono::steady_clock::now()-started).count();
 ...
 cerr<<"WALL="<<wall<<" attempts="<<attempts<<" attemptsPerSec="<<(attempts/wall)<<" fallbackKeys="<<nFallback<<"\n";
