...
int main(int argc,char**argv){
 ...
 long long nExp=0,nSortItems=0,nSorts=0,nBuckets=0;double tExp=0,tHash=0,tSort=0,tEv=0;
  ...
  for(int trial=0;trial<8;trial++){
   ...
   for(int t=0;t<(int)ids.size()&&!states.empty();t++){
    ...
    for(auto&s:states)for(auto&op:options){
     ...
     nExp++;next.push_back(n);candBuf.push_back({n.rank,(int)next.size()-1,n.k});candKey.push_back(bucket_key(n));
    }
    ...
    if(next.size()>(size_t)beam){nSorts++;nSortItems+=next.size();
     ...
     tHash+=chrono::duration<double>(chrono::steady_clock::now()-__q1).count();
 ...
 cerr<<"PROF attempts="<<attempts<<" expPerAttempt="<<(nExp/(double)attempts)<<" avgSortItems="<<(nSorts?(double)nSortItems/nSorts:0.)
     ...
     <<"     expansionSec="<<tExp<<" ("<<(tExp/seconds*100)<<"%)  hashSec="<<tHash<<" ("<<(tHash/seconds*100)
     <<"%)  nth+sortSec="<<tSort<<" ("<<(tSort/seconds*100)<<"%)  evalSec="<<tEv<<" ("<<(tEv/seconds*100)
     <<"%)  other="<<(seconds-tExp-tHash-tSort-tEv)<<"\n";
