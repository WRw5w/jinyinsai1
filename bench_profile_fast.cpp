...
struct Cand{double r;int i;};          // 16 B, contiguous rank comparisons
...
int main(int argc,char**argv){
 ...
 if(!in)return 3;
 vector<Cand>candBuf;vector<State>keptBuf;
 double seconds=stod(argv[3]);
 long long nExp=0,nSortItems=0,nSorts=0,nLevels=0;double tExp=0,tSort=0,tDedup=0;mt19937 rng(stoul(argv[4]));int beam=stoi(argv[5]);long attempts=0,accepted=0;
  ...
  for(int trial=0;trial<8;trial++){
   ...
   for(int t=0;t<(int)ids.size()&&!states.empty();t++){
    ...
    for(auto&s:states)for(auto&op:options){
     ...
     nExp++;next.push_back(n);candBuf.push_back({n.rank,(int)next.size()-1});
    }
    ...
    if(next.size()>(size_t)beam){nSorts++;nSortItems+=next.size();
     ...
     // Same reduction as before, but the sort orders 16-byte (rank,index) records
     // instead of 120-byte States, and the bucket dedup is a fixed-size linear scan
     // instead of a std::set. Both are exact: the key packing is injective, so the
     // survivor set and their order are identical.
     sort(candBuf.begin(),candBuf.end(),[](const Cand&a,const Cand&b){return a.r<b.r;});
     tSort+=chrono::duration<double>(chrono::steady_clock::now()-__q0).count();
     auto __q1=chrono::steady_clock::now();
     keptBuf.clear();
     long long bkeys[80];int bn=0;
     for(const Cand&c:candBuf){
      const State&s=next[c.i];
      long long key=((long long)(int)(s.l1*2)<<32)^(unsigned)(int)(s.l2*2);
      bool dup=false;for(int j=0;j<bn;j++)if(bkeys[j]==key){dup=true;break;}
      if(dup)continue;
      bkeys[bn++]=key;
      keptBuf.push_back(s);
      if(keptBuf.size()>=(size_t)beam)break;
     }
 ...
 cerr<<"PROF attempts="<<attempts<<" expPerAttempt="<<(nExp/(double)attempts)<<" avgSortItems="<<(nSorts?(double)nSortItems/nSorts:0.)<<"\n"     <<"     expansionSec="<<tExp<<" ("<<(tExp/seconds*100)<<"%)  sortSec="<<tSort<<" ("<<(tSort/seconds*100)<<"%)  dedupSec="<<tDedup<<" ("<<(tDedup/seconds*100)<<"%)  other="<<(seconds-tExp-tSort-tDedup)<<"\n";
