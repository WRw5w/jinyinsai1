...
def run(args):
    ...
    plan=json.loads(source.read_text(encoding='utf-8'));before=validate_plan(plan,orders,cfg,blanks);ids={o.oid:i for i,o in enumerate(orders)}
    ...
    with inp.open('w') as f:
        ...
        for i,o in enumerate(orders):f.write(f'{o.pieces} {model.caps[i]} {o.size:.12f} {o.linear_weight:.15f} {model.parallel_limit((i,))}\n')
    ...
    after=validate_plan(plan,orders,cfg,blanks);independent=check(plan)
    if not independent['passed']:raise ValueError(independent['error_counts'])
    if after['platform_score_estimate']<before['platform_score_estimate']-1e-8:raise ValueError('Search regressed')
    ...
    report=dict(before=before,after=after,independent_check=independent,elapsed_seconds=time.perf_counter()-started,official_acceptance_verified=False,seed=args.seed,beam=args.beam)
...
if __name__=='__main__':
    ...
    ap.add_argument('--output',default='runs/normal_heterogeneous')
    ap.add_argument('--engine',default='heterogeneous_search.exe')
