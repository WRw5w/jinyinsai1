...
| # | 目录 | 预测（floor） | 方案数 | 作用 |
|---|---|---:|---:|---|
| 1 | `cand_rounded` | **92.0120** | 1880 | 最佳候选；长度写成精确小数 |
| 2 | `cand_compliant` | 91.3965 | 1880 | **1 的对照**——同一方案，长度写成浮点乘积 |
| 3 | `cand_covprobe25` | 91.8167 | 1881 | 覆盖率探针（拆 25% 的多订单方案） |
| 4 | `cand_covprobe` | 91.2725 | 1886 | 覆盖率探针（拆 50%） |
| 5 | `cand_yieldprobe` | 88.8129 | 1880 | 权重探针（坯料多报 5%） |
...
- 榜面分**有差** ⇒ 平台用 **floor**（那么 1 是更优包）
- 榜面分**无差** ⇒ 平台用 **snap**
- 差值的**大小**同时定出基准刀数：在 160,000 下差 0.61 分，在 90,000 下差 0.34 分。

**3 与 4** —— 覆盖率各降 0.97 / 3.62 个点，其余不变。分数降幅 ÷ 覆盖率降幅 = 覆盖率权重（应为 0.20）。同时验证覆盖率的口径（"仅统计多订单组合方案中的订单"是否按我们理解计算）。

**5** —— 成材率降 10.66 个点，其余不变。分数降幅 ÷ 成材率降幅 = 成材率权重：**0.30 还是 0.40**。官方 PDF 的权重表写 30%，同一页的公式行写 40%，两者矛盾，这是唯一能定它的办法。
