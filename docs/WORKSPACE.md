...
历史审查脚本仍保留旧算法用于复算，整理没有修正它们的推断；见 [证据边界](EVIDENCE.md)。

## 验证

- 复赛原 64 项回归在整理前有 1 项失败：缺少未入库 runs/platform_fix/result.json。
- 整理后 67 项：66 通过，1 项可选历史运行缺失而显式跳过；新增 3 项验证跨目录入口、样本哈希和打包目标。
- 13 个带参数帮助的运行入口从项目外目录启动通过。
- 383 个归档文件实际解压后逐项哈希匹配，101 个保留/迁移映射均有实际文件。
- MCP 仅将根部 test_auto_submit.py 移到 tests 并调整导入路径；完整 35 项离线测试通过，服务/插件入口未移动。
