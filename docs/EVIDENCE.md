...
计数于 2026-09-26 从远端固定版本的 JSON/ZIP 独立复算；候选式已用于 `aic.py audit-clause6` 诊断，未并入生产校验器。
...
- 生产校验器仍使用 B；七包现行报告及 `resync` 已明确禁止放行，原 passed/0 违规报告另存为 `validation_report_legacy_B.json`。新打包流程和 MCP 的完整机器闸门仍需后续修复。
- `tools/analysis/verify_clause6_readings.py` 的副本检查只比较 B 计数；它不能验证完整 JSON 内容一致，读取错误也可能被跳过。
