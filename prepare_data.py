...
def prepare(root=Path('data')):
    ...
    report = {'source_orders': valid + len(rejected), 'valid_orders': valid, 'rejected': rejected,
              ...
              'confirmed_from_platform_feedback': ['length_scheme stores net integer multiples without trim',
                                                   'physical round length is sum(length_scheme)+2m'],
              'assumptions': ['One shared 1 metre trim at each round end; never added to length_scheme',
