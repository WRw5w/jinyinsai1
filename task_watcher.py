...
def main():
    ...
    hashes = {}
    for source in ['solver.py', 'competition_solver.py', 'task_watcher.py',
                   'optimize_patterns.py', 'pattern_dp.cpp', 'pattern_dp.exe',
                   'refine_patterns.py', 'pair_refine.py', 'platform_check.py',
                   'data/orders.normalized.csv',
                   'data/blanks.normalized.csv', 'data/competition.config.json', 'data/data_audit.json']:
        path = Path(source)
        if path.is_file():
